export const createContainerBlock = (elements: HTMLElement[]) => {
  ...
  output.classList.add("dtb-mockup-code");
  elements.forEach((element) => {
    ...
    copyToClipboardOriginal.forEach((e) => e.remove());
  });
  ...
  copyButton.classList.add(
    ...
    "dtb-relative",
    "dtb-opacity-100",
    "docsify-terminal-copy-button"
  );
  const copyButtonIcon = document.createElement("svg");
  copyButtonIcon.classList.add("dtb-w-5", "dtb-h-5", "dtb-fill-current");
